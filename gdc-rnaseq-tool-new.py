...
def arg_parse():
    ...
    parser.add_argument('gene_counts_dir', action="store", help='Path to gene_counts tsv files')
    parser.add_argument('save_dir', action='store', help='where to save the Merged_FPKM.tsv file')
...
def main(args):
    ...
    global hugo
    global gene_counts_dir
    global save_dir
    manifest_file = args.manifest_file
    hugo = args.hugo
    gene_counts_dir = args.gene_counts_dir
...
Location = save_dir  # 'Merged_RNASeq_' + timestr + '/'  # Create path object from the directory
...
Matrix = {}
import glob
Files = glob.glob(os.path.join(gene_counts_dir, '*.tsv'))
...
for line in lines[6:]:
    splits = line.split('\t')
    gene_ids.append(splits[0])
    gene_names.append(splits[1])
Matrix['gene_id'] = gene_ids
Matrix['gene_name'] = gene_names
for file in Files:
    ...
    with open(file, 'r') as fp:
        lines = fp.readlines()
    values = [float(line.split('\t')[7]) for line in lines[6:]]
    print(Name)
    Matrix[Name] = tuple(values)
...
if len(Matrix) > 0:
    Merged_File_Name = 'Merged_FPKM.tsv'
    Counts_Final_Df = pd.DataFrame(Matrix)
    Counts_Final_Df.set_index('gene_id')
